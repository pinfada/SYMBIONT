export function getContextualMurmure(context: { hour: number; firstLogin?: boolean }, step?: string): string {
  if (step === 'intro') {
    if (context.hour > 22 || context.hour < 6) return 'Il est tard… le réseau sommeille, mais une lueur veille encore.';
    if (context.firstLogin) return 'Un organisme attend d’être éveillé…';
