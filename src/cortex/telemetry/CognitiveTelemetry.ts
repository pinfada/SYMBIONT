...
export class CognitiveTelemetry {
  ...
  async log(
    ...
    // Les champs optionnels ne sont inclus que s'ils sont définis
    // (exactOptionalPropertyTypes interdit les valeurs `undefined` explicites)
    const entry: CognitiveLogEntry = {
      ...
      ...(details?.stateTransition !== undefined && {
        stateTransition: details.stateTransition as { from: CortexState; to: CortexState },
      }),
